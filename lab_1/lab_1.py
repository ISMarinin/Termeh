...
def anima(j):
    ...
    # Rline.set_data([X[j], X[j] - Rho[j] * (-VY[j]) / math.sqrt(math.pow(-VX[j], 2) + math.pow(-VY[j], 2))],
    #                [Y[j], Y[j] + Rho[j] * (-VX[j]) / math.sqrt(math.pow(-VX[j], 2) + math.pow(-VY[j], 2))])
